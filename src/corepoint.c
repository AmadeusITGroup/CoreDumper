...
int Coredumper_PutCorePointAt(void* iAddress __attribute__((unused)), const char* iFileName __attribute__((unused)), const struct CoreDumpParameters* iParameters __attribute__((unused)))
{
  errno = EINVAL;
  return -1;
}

int Coredumper_DeleteCorePointAt(void* iAddress __attribute__((unused)))
{
  errno = EINVAL;
  return -1;
}
